...
5. **PARTIAL — service refunds and external service accounting.** Creator Balance Preferred/Ad Credit purchases have a distinct service ledger. Automated service refunds are deliberately absent because entitlement-unwind eligibility is unsettled. The pre-existing Stripe Ad Credit fulfillment path issues credits but does not populate the new unified service-purchase/revenue ledger, so operator service reporting is not yet source-complete.
...
7. **PARTIAL — Preferred billing.** Creator Balance can explicitly pay the settled monthly or annual charge and paid coverage controls tier benefits. Automatic recurring balance consumption is correctly deferred. A general Creator-facing external Preferred checkout and production recurrence/fallback path do not exist.
...
| Preferred Creator Balance payment | **COMPLETE** | Explicit $20 monthly coverage or $200 annual coverage, service debit/revenue, zero processor fee, term/coverage linkage, idempotency and duplicate-period rejection are atomic. No split tender. |
| Preferred recurring/external billing | **PARTIAL / DEFERRED BY POLICY** | No silent balance recurrence is correct. There is no general Creator-facing external Preferred checkout or production recurrence/fallback implementation. |
...
| Stripe Ad Credit accounting | **PARTIAL** | Existing Stripe fulfillment creates `creator_ad_credit_purchases` and credit ledger entries. It does not create a corresponding migration-034 unified service purchase/revenue record, so new operator service reporting is incomplete for external payments. |
| Service refunds/reversals | **DEFERRED BY POLICY / NEEDS OWNER DECISION** | No liberal service refund policy was invented. Unused-credit refund eligibility and Preferred entitlement unwind rules need a settled decision before mechanics. |
...
4. Decide service-refund eligibility and unwind rules before adding refund controls.
...
| **PARTIAL** | Continuing Creator hard gate, additional identities, bundles/sales polish, general order history, Preferred external/recurring billing, Stripe service accounting, product refunds/disputes/payout execution, operator UI breadth, notices, accessibility |
...
The correct next step is a focused remediation pass beginning with the continuing Creator hard gate, followed by a policy decision on coupons and service refunds, then additional-identity billing and unified external service accounting. Production must remain closed until legal and operational blockers are resolved and the remediated system passes the full suite plus direct negative authorization tests.
