...
export async function handleAccountLibraryRequest(
  request,
  env = {},
  options = {},
) {
    ...
    return json(
      {
        error: {
          ...
          message: "Use GET for My Library.",
        },
      },
      405,
    );
    ...
    return json(
      {
        error: {
          ...
          message: "Sign in to view My Library.",
        },
      },
      401,
    );
  ...
  try {
    rows = await allRows(
      database
        .prepare(
          `
    SELECT o.public_id, o.created_at, o.payment_source, oi.product_slug, oi.product_title_snapshot,
    ...
    if (
      !/no such (table:\s*(creator_listings|marketplace_creators)|column:\s*o\.payment_source)/i.test(
        String(error),
      )
    )
      throw error;
  ...
  const secret = String(
    options.downloadSigningSecret || env.DOWNLOAD_SIGNING_SECRET || "",
  );
  ...
  for (const row of rows) {
    ...
    items.push({
      ...
      downloadAvailable: Boolean(
        row.entitlement_id && row.entitlement_status === "active",
      ),
      downloadUrl,
      orderReference: row.public_id,
      paymentDescription:
        row.payment_source === "creator_balance"
          ? "Paid with Creator Balance"
          : "Paid through secure checkout",
